...
import { GamePlayer, Game } from 'dart3-sdk';
...
export class GamePlayerRepository {
  ...
  async create(ctx: Context, gameId: number, bet: number, playerId: number): Promise<number> {
    const [response, err] = await transaction([
      {
        query: `INSERT INTO game_player (game_id, player_id) values($1, $2) RETURNING id;`,
